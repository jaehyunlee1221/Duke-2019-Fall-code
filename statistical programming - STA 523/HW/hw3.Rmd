...
```{r US map with count for unsolved homicide, fig.cap = "Number of unsolved homicide by states"}
state2 <- state %>%
...
```{r unsolved-homicide-count, fig.cap = "Time series trend of unsolved homice in three states"}
...
```{r weapon-assign-state-animated}
ggplot(data=unsolved_homicide_count2, aes(x=year, fill = factor(gun, labels = c("Firearm not used as weapon", "Firearm used as weapon")))) + 
  geom_bar(position = "fill") + 
  facet_grid(~state) +
...
```{r proportion of weapon types, animation.hook="gifski", interval=0.5, cache=TRUE, fig.cap="Proportion of weapon type used in three state over years"}
