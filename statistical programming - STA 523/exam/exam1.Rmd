...
```{r data flatten}
# Function for flatten competitor and teams and make them into data.frame
org_df <- function(data){
  temp <- map_dfr(map(data, unlist), as.list)
  return(temp)
}
# Function for detecting nest list and convert it into data.frame
org_ls_df <- function(var){
  if(typeof(var) == "list") var <- org_df(var)
  return(var)
}
# Function for merging team and competitors
org_merge <- function(temp1,temp2){
  if(length(temp1) > length(temp2)) colnames(temp2) <- paste("team", colnames(temp2), 
                                                             sep = ".") else
    colnames(temp1) <- paste("team",colnames(temp1),sep = ".")
  result <- full_join(temp1,temp2)
  return(result)
}
# Function for combining merged dataframe with others.
...
# Function for flattening whole list as dataframe
org_fin <- function(x){
...
```{r select and change variable name}
# Select variable and change their name and type.
tdf_df_task1 <- tdf_df %>% 
  mutate(rider_name = name, rider_nat = nationality, team_name = team.name, 
         ...
         young_rider_time = result.young_rider, 
         young_rider_rank = result.young_rider_ranking) %>% 
  select(rider_name, rider_nat, team_name, team_nat, stage, dep_city, arr_city,
         classification, distance, start_date, time, time_rank, sprint_pts, sprint_rank,
         climb_pts, climb_rank, young_rider_time, young_rider_rank)
...
# Function for summing time integer form.
sum_time <- function(x){
  num1 <- na.omit(x)[1]
...
# Function for deviding time into each elements and summing into time format.
...
```{r spread stage with value of climber pts}
...
#### Code {- .tabset .tabset-fade .tabset-pills}
```{r time_sum function}
# time sum function. Not like above function, this function is made for summing total time
time_sum <- function(x){
...
```{r data handling for drawing plots}
# Progress for allocating overall ranking 
...
# Since stage 2 is team_time_trial, fill NA as team-time for individual time record.
