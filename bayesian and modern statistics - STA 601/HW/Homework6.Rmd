...
For possion model used at Exercise 4.8, we will parameterize $\theta_A$ and $\theta_B$ as $\theta_A = \theta$, $\theta_B = \theta \gamma$. In this parameterization, $\gamma$ represent the relative rate. 
Let $\theta \sim gamma(a_\gamma, b_\gamma)$ and $\gamma \sim gamma(a_\gamma,b_\gamma)$
...
$$
\begin{aligned}
P(\theta \mid y_A, y_B, \gamma) \propto P(\theta \mid \gamma)P(y_A,y_B \mid \theta, \gamma) &= P(\theta)P(y_A,y_B \mid \theta, \gamma) \\
...
$$
\begin{aligned}
&P(Z_i \mid \beta, x_i) \sim  \; N(\beta x_i, 1) \\
\rightarrow &P(\mathbf{z} \mid \beta, \mathbf{x}) = P(Z_1 \cdot\cdot\cdot Z_n \mid \beta\mathbf{x}) = (2\pi)^{-n/2}exp\{\frac{\sum_{i=1}^{n}(Z_i - \beta x_i)^2}{2}\}
\end{aligned}
$$
$$
\begin{aligned}
...
$$
\begin{aligned}
P(\beta \mid \mathbf{x,y,z},c) &\propto P(\mathbf{y} \mid \beta, \mathbf{x,z},c) P(\mathbf{z} \mid \beta, \mathbf{x}, c)P(\beta \mid \mathbf{x},c) \\
&\propto P(\mathbf{z} \mid \mathbf{x},\beta,c)P(\beta) \\
...
show that full conditional distribution of c is constrained normal density. Similarly show that full conditional distribution of $z_i$ is proportional to a normal density but constrained to be either above c or below c depending on $y_i$
...
&= \Phi(c - \beta x_i)P(z_i \mid \beta, x_i,c)
\end{aligned}
$$
...
acf(sample$beta)
```
Autocorrelation between lags in beta b is very high.

```{r}
acf(sample$c)
```
Autocorrelation between lags in c is also very high.
...
In both measure, orange crab is larger than blue crab on average. As an evidence, the predictive probability that orange crab is larger than blue crab in both width and depth shows that 90% and 68% for each measure.
...
ggplot() +
  geom_histogram(mapping = aes(x = blue.cor), col = "blue", fill = "skyblue") +
  labs(title = "Density of corrletion in blue.crab", x = "corrleation")
ggplot() +
  geom_histogram(mapping = aes(x = orange.cor), col = "red", fill = "orange") +
